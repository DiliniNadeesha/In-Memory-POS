...
public class PlaceOrderFormController {
    ...
    public void initialize() {
        ...
        ObservableList<CustomerTM> customers = FXCollections.observableList(ManageCustomerFormController.customersDB);
        ...
        cmbCustomerId.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<CustomerTM>()
                ...
                // Start of the Anonymus Inner Class
        {
            @Override
        ...
        // End of the Anonymus Inner Class
        ...
        // When user selects a Item Codes
    ...
    public void btnPlaceOrder_OnAction(ActionEvent actionEvent) {
        ...
        if (tblOrderDetails.getItems().size() == 0) {
            new Alert(Alert.AlertType.ERROR, "Ubata pissuda yako, nikan order dannea (You can't place an order)", ButtonType.OK).show();
            cmbItemCode.requestFocus();
            return;
        }
        ...
        Order newOrder = new Order(lblId.getText(),
                LocalDate.now(),
                cmbCustomerId.getValue().getId(),
                alOrderDetails);
        ordersDB.add(newOrder);

        new Alert(Alert.AlertType.INFORMATION, "You're work is completely done", ButtonType.OK).showAndWait();
    ...
    private void updateStockQty(String itemCode, int qty) {
        for (ItemTM item : ManageItemFormController.itemsDB) {
